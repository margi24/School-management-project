...
public class UI {
    ...
    private void updateStudent() {
        ...
        Optional<Student> student1 = service.updateStudent(student);
        if (student1 == null) {
    ...
    private void stergeTema() {
        ...
        String id = scanner.next();
        Optional<Tema> tema = service.deleteTema(id);
        if (tema == null) {
    ...
    private void updateTema() {
        ...
        Optional<Tema> tema1 = service.updateTema(tema);
        if (tema1 == null) {
            System.out.println("Tema nu exista!");
        } else {
    ...
    private void adaugaNota() throws ValidationException {
        ...
        Nota notaMax = new Nota(idNota, idStudent, nrTema, 10, dataPredare);
        int predare = service.calculeazaSPredare(notaMax.getData());
        Tema tema = service.findTema(notaMax.getIdTema());
        if(predare != tema.getDeadline()) {
            if (predare - tema.getDeadline() == 1) {
                notaMax.setNota(notaMax.getNota() - 2.5);
            } else if (predare - tema.getDeadline() == 2) {
                notaMax.setNota(notaMax.getNota() - 5);
            }
        }

        System.out.println("Nota maxima pe care o poate primi studentul este: " + notaMax.getNota());
       ...
       // System.out.println("Nota primita este: " + notaFinala);
    }
*/
    /**
     * Sterge o nota
    ...
    private void stergeNota() {
        ...
        String nrTema = scanner.next();
        String idNota = idStudent + "#" + nrTema;
        Optional<Nota> nota = service.deleteNota(idNota);
        if (nota == null) {
